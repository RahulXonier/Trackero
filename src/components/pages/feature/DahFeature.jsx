...
const DahFeature = () => {
    return (
        <div className='w-full min-h-[80vh] py-20 bg-green '>
            <div className='max-w-7xl h-full mx-auto grid gap-12 px-4 '>
                <div className='flex flex-col justify-center items-center'>
                    <span className=" flex  items-center gap-2 px-4 py-1.5 bg-white border border-[#e3ebf2] rounded-full text-[13.5px] font-semibold text-[#0fb8c4] shadow-sm mb-[18px] w-fit">
                    ...
                    <p className='max-w-150 text-center text-[17px] text-sm mt-[14px] text-[var(--ink-500)] '>Our dashboard gives you a complete overview of your operations with powerful insights and intuitive visuals.</p>
                </div>

                <div className='grid md:grid-col-2 lg:grid-cols-3 gap-10'>
                    {
                        data.map((i) => {
                            return (
                                <div key={i.id} className='border relative h-[200px] overflow-hidden group  rounded-2xl justify-center items-center p-4 flex flex-col gap-6' style={{ borderColor: i.cardBorder }}>
                                    <div
    ...
    group-hover:translate-y-0 transition-all duration-700  text-[var(--ink-500)]'>{i.description}</p>
                                    </div>
